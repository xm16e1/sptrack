================================================================================
MICROSOFT FOUNDATION CLASS ライブラリ: ImageConvDialog プロジェクトの概要
===============================================================================

アプリケーション ウィザードが作成した ImageConvDialog には Microsoft 
Foundation Class の基本的な使い方が示されています。アプリケーション作成のひな型
としてお使いください。

このファイルには ImageConvDialog アプリケーションを構成している各ファイ
ルの概要説明が含まれます。

ImageConvDialog.vcproj
...
ImageConvDialog.h
   ...
   CImageConvDialogApp アプリケーション クラスを宣言します。

ImageConvDialog.cpp
   これは、アプリケーション クラス CImageConvDialogApp を含むメイン アプリケー
   ション ソース ファイルです。
...
res\ImageConvDialog.ico
   これは、アプリケーションのアイコンとして使用されるアイコンファイルです。この
   アイコンはメイン リソース ファイル ImageConvDialog.rc に含まれていま
   す。

res\ImageConvDialog.rc2
...
ImageConvDialogDlg.h, ImageConvDialogDlg.cpp - ダイアログ
   これらのファイルは CImageConvDialogDlg クラスを含みます。このクラスはアプ
   リケーションのメイン ダイアログの動作を定義します。ダイアログ テンプレートは
   Microsoft Visual C++ で編集可能な ImageConvDialog.rc に含まれます。
...
StdAfx.h, StdAfx.cpp
   ...
   ImageConvDialog.pch や既にコンパイルされた型のファイル StdAfx.obj を
   ビルドするために使用されます。
